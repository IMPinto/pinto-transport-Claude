    ...
    <div className="bg-gray-900 rounded-lg p-6 border border-gray-800">
      ...
      <div className="space-y-4">
        {recentRoutes.map((route) => (
          <div key={route.id} className="flex items-center justify-between p-4 bg-gray-800 rounded-lg">
            <div className="flex items-center space-x-4 space-x-reverse">
              <div className="w-10 h-10 bg-primary/20 rounded-lg flex items-center justify-center">
                <Clock className="w-5 h-5 text-primary" />
              </div>
              <div>
                <h4 className="text-sm font-medium text-text">{route.description}</h4>
            ...
            <div>
              <span className={`px-2 py-1 text-xs rounded-full ${
                route.status === 'פעיל' ? 'bg-green-900/50 text-green-400' :
                route.status === 'הושלם' ? 'bg-blue-900/50 text-blue-400' :
